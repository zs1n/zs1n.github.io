...
En el código fuente muestra la siguiente cadena en `base64`.
...
Si la codifico quizás tenga algo de información.
...
Antes de pasar a la enumeración de directorios, y demás, es de notar que corre un `IIS (Internet Information Service)` con la versión `8.5`.
...
Además por detrás corre `ASP.NET`, por lo que puedo pensar además de `fuzzear` por directorios, también por archivos con extensiones como `asp` o `aspx` que corresponder a archivos de `.NET`.
...
🚨 Caught ctrl+c 🚨 saving scan state to ferox-http_10_129_255_40_62696_-1765315726.state ...
...
Como parece que espera el parámetro `u` se lo indico, el cual parece que después del mismo poder pasarle una `url`, podría montar un servidor en `Python` con el siguiente comando para que desde la url pueda visualizar mis archivos.
...
Pero parece que la pagina me da error `500`, por lo que podría ya que me pide una `url` especificar el mismo `localhost` de la maquina.
...
Ya desde acá lo que me llama la atención es el link hacia `system commands` el cual apunta a un archivo `cmd.aspx`, pero si clickeo en el me lleva a mi propio localhost.

![image-center](/assets/images/Pasted image 20251209183743.png)
## Command

Para poder tener resolución puedo usar el mismo `SSRF` para poder ver el archivo.
...
Ya que apunta a la `ip` de la maquina y a ese archivo el cual externamente no esta expuesto, pero otra vez mas, aprovechando de la vulnerabilidad puedo usar los típicos parámetros en archivos de este tipo como `?c`, `?cmd`, `?command`, entre otros, pero en este caso en el código fuente me indica de que manera puedo ejecutar.
...
Veo que el parámetro es `xcmd` por lo que en la misma url puedo indicar algo como esto:
...
La respuesta es rara, ya que no me muestra el `output`, de los comandos como usualmente en este tipo de shells se ve, sino un código de esta pareciera ser, probando veo que es cierto, ya que si por ejemplo si coloco el comando `whoam`, obtengo esta respuesta.

![image-center](/assets/images/Pasted image 20251209184551.png)

Código de estado `1` el cual es falso, podría intentar enviarme una shell o descargarme el `nc.exe`.

![image-center](/assets/images/Pasted image 20251209184816.png)

Sin embargo, ni siquiera recibo la solicitud y además el código de estado es `1` por lo que quizás no sea un comando invalido, sino hay algo por detrás que impide descargar archivos en la maquina. Ahora quiero ver si tengo `ping` hacia mi maquina, lo hago desde `curl` par jugar mas cómodamente.
...
Por lo que puedo pensar en que shell por `TCP` no acepta, pero quizás por `ICMP` si.
Por lo que me cree un script en `Python` que lo que hace tomando de referencia este [archivo](https://github.com/samratashok/nishang/blob/master/Shells/Invoke-PowerShellIcmp.ps1) en `Powershell` para peticiones `ICMP`, que lo que hace es enviar el payload en codificado en `base64` en fragmentos, después, en la función `listener()` crea un listener, el cual solo filtra los paquetes `ICMP` únicamente de la ip victima luego recibe las repuestas o peticiones `ICMP` en partes, que luego en la función `process_packet()`, construye todos los fragmentos devolviendo así el output en la terminal.
...
Veo que en la raíz esta el directorio `sysadmscripts`, dentro del cual están los siguientes archivos.
...
Es así como después de unos segundos, los archivos están dentro de la ruta que especifique, además perdí mucho tiempo tratando de analizar el contenido del `backup` y no tiene nada de nada, por lo que trate de ver las `ADS (Alternate Data Strams)` que los archivos que conseguí tenían.

>Las **Alternate Data Streams (ADS)**, o Flujos de Datos Alternativos, son una característica del sistema de archivos `NTFS` de Windows que permite adjuntar datos adicionales a un archivo de forma oculta, sin que cambie su tamaño visible ni sea detectado por el explorador de archivos común, funcionando como un "archivo dentro de otro archivo", usados para metadatos legítimos o para ocultar malware y evadir seguridad
...
Como vi que habían muchos puertos abiertos localmente, quise intentar abrirlos, ya que las credenciales eran validas para el usuario `Administrator`.
...
Ahora para correrla uso el parámetro `/run`.
...
Para deshabilitar el firewall use el siguiente comando, basándome en la documentación de [Microsoft](https://learn.microsoft.com/en-us/windows/security/operating-system-security/network-security/windows-firewall/configure-with-command-line?tabs=cmd)
...
Por lo que ahora puedo conectarme vía `WinRM`, usando las credenciales del usuario `Administrator`.
