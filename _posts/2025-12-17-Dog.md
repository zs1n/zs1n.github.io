...
Buscando por vulnerabilidades, veo una que se trata de una vez estar autenticados en la pagina, tengo la posibilidad de obtener una ejecución remota de comandos.
...
Si ejecuto el comando status de `bee`. Veo que me sale un mensaje el cual dice que no se encuentra la instalacion de `Backdrop`, quiza es porque no estoy en el directorio adecuado, y como se que la pagina corre en `/var/www/html` puedo ir a esa ruta y ejecutar algún comando desde ahi.
