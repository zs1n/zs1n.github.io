...
`Mentor` es una máquina `Linux` de dificultad media cuya ruta incluye pivotar a través de cuatro usuarios diferentes antes de llegar a `root`. Después de escanear un servicio `SNMP` con una cadena comunitaria que puede ser forzada bruta, se descubren credenciales de texto plano que se utilizan para un punto final `API`, que resulta ser vulnerable a la ejecución de código remoto ciego y conduce a un punto de apoyo en un contenedor de `docker`. Enumerar la red de containers revela un servicio `PostgreSQL` en otro contenedor, que se puede aprovechar en `RCE` mediante la autenticación utilizando credenciales predeterminadas. Examinar una copia de seguridad de base de datos antigua en el contenedor PostgreSQL revela un `hash`, que una vez agrietado se utiliza para `SSH` en la máquina. Finalmente, al examinar los archivos de configuración en el host, el atacante puede recuperar una contraseña para el usuario `james`, que puede ejecutar el comando `/bin/sh` con privilegios sudo, perdiendo instantáneamente los privilegios `root`.
...
De por si el escaneo no revela nada interesante ya que la única ruta me devuelve un `Forbidden`.
...
───────────────────────────┬──────────────────────
 ...
 🚩  In-Scope Url          │ api.mentorquotes.htb
 🚀  Threads               │ 50
 📖  Wordlist              │ /usr/share/seclists/Discovery/Web-Content/raft-medium-directories.txt
 👌  Status Codes          │ All Status Codes!
 💥  Timeout (secs)        │ 7
...
En la ruta docs veo el comportamiento de la api donde puedo ejecutar acciones como el logueo, y la autenticación entre otras mas.

![image-center](/assets/images/{D0AE0B2E-3245-4084-A677-4BA1210F1505}.png)

#### auth/login

En el apartado de `auth/login` si toco en el botón en la parte superior que dice `Try it out` me sale una interfaz en la cual puedo comprobar el envio de datos y como se veria la respuesta.
...
Quizás sea porque no estoy logueado, pero si me voy a la sección de `/auth/signup` puedo registrar al usuario por las dudas pruebo de nuevo con credenciales.

![image-center](/assets/images/{8531D6C3-9772-495A-BCD3-50D2664F9146}.png)

Es así como veo un token de sesión por lo que parece. el cual puedo usarlo en el endpoint `/users/` por el metodo `GET`. Intercepte la peticion donde tengo que agregar el `Header` `Authorization` con el dato del token
...
Sin nada mas decidí `fuzzear` por el directorio `admin/`.
...
🚨 Caught ctrl+c 🚨 saving scan state to ferox-http_api_mentorquotes_htb_admin-1764289507.state ...
[#######>------------] - 2m     11991/30000   3m      found:3       errors:10     
[###>----------------] - 2m      5979/30000   45/s    http://api.mentorquotes.htb/admin/
```

Después de un rato veo las rutas `/check` y `/backup` , `check` me daba un mensaje irrelevante y después en `backup` puse los datos el usuario `admin` y me salió el siguiente error.
...
Pareciera como si se realizara un comando por detrás el cual no se o quizás la ejecución de un script, por lo que se me ocurre una inyección de comandos en alguno de los valores.

Probando con una inyeccion como esta. Aclaración: Saque los demás valores ya que testeando me di cuenta q no hacían falta
...
El siguiente archivo destaca entre los demás debido a que se ve como se realiza la conexión a una base de datos `Postgresql`, en otro contenedor por lo que parece.
...
Podría tratar de conectarme yo mismo debido a que en el contenedor escasean las `tools` como `psql` para poder realizar la conexión a la base de datos. Por lo que subí `chisel` para poder entablar un túnel `SOCKS5`, con el siguiente comando.
...
Por lo que ya puedo usar `proxychains` para poder conectarme al otro contenedor y así a su base de datos.
...
Como veo dos hashes puedo ir a la pagina [Crackstation](https://crackstation.net/) para poder romperlos ya que son fáciles, por el hecho de que son `md5`.

![image-center](/assets/images/{4DDB8B89-EC6F-4878-828D-E39DE9FCF10E}.png)

Una vez tengo la pass del usuario `svc` me conecto vía `ssh`.
...
Ya dentro de la maquina como `svc` no veo nada interesante con mi enumeración manual, así que subí `linpeas.sh` a la maquina para poder ver via de escalada a `root` o al usuario `james`.
...
2025-11-28 05:54:01 (334 KB/s) - ‘linpeas.sh’ saved [954437/954437]
```

Después de un análisis del output veo un archivo de configuración del protocolo `snmp`. Lo cual tiene sentido sabiendo que enumere ese servicio externamente.
### snmp.conf
