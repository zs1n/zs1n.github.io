...
De por si no muestra mucho, además en el pie de la pagina veo que al parecer esta corriendo con `Searchor` en la versión `2.4.0`. Antes de buscar por algún tipo de vulnerabilidad voy a `fuzzear` por si encuentro algo interesante
...
🚨 Caught ctrl+c 🚨 saving scan state to ferox-http_searcher_htb_-1764263975.state ...
...
Así que con las mismas me voy a loguearme directo a este servicio en la url `http://gitea.searcher.htb`. Sin emabrgo no encuentro nada de nada. 
...
Si lo ejecuto usando el parámetro `--help` veo el siguiente panel de ayuda.
...
Veo lo que parecen ser credenciales, las cuales podrían ser del usuario `administrator` en `gitea`, probando las mismas veo que son validas y ademas veo que tiene acceso a un repositorio llamado `scripts`.
