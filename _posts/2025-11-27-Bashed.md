...
`Bashed` es una máquina bastante `fácil` que se centra principalmente en la fluidez y la localización de archivos `importantes`. Como el acceso básico al `crontab` está restringido, recurrimos a herramientas de monitoreo de procesos como `pspy` para poder descubrir procesos críticos, los cuales derivan es una escalada de privilegios.
# Reconocimiento
...
La web de por si no muestra nada de nada, así que sigo por `fuzzing`
...
🚨 Caught ctrl+c 🚨 saving scan state to ferox-http_bashed_htb_-1764227325.state ...
...
Después de correr el `linpeas.sh` el cual se puede descargar desde este [enlace](https://github.com/peass-ng/PEASS-ng/releases) no descubrí nada, por lo que descargue también el `pspy` que también se puede descargar desde este [repositorio](https://github.com/DominicBreuker/pspy/releases/tag/v1.2.1).
### pspy

Corriendo el binario, me di cuenta que por detrás corre una tarea `cron` la cual hace que cualquier script en `python` se ejecute con alto privilegios.
...
Por lo que se me ocurre crear un script el cual ejecute un comando el cual voy a controlar desde el mismo código. En mi caso para hacerlo simple voy a hacer que una vez que el script corra se le asigne el bit `SUID` a la `bash`, de manera que el script final quedaría así.
...
Y ahora ya puedo generar el script el cual en su forma final quedaría así:
...
Por lo que ya ahora me puedo convertir en `root` ejecutando el comando `bash -p` para convertir mi bash de bajos privilegios a una que tenga los `máximos` privilegios del sistema.
