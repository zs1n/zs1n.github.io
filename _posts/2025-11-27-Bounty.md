...
`Bounty` es una máquina de dificultad `fácil` a media, que cuenta con una técnica interesante para `eludir` las protecciones del cargador de archivos y lograr la ejecución de código. Esta máquina también destaca la importancia de mantener los sistemas `actualizados` con los últimos parches de seguridad.
# Reconocimiento
...
El `fuzzeo` no revela nada interesante. Ahora realizo la misma para la búsqueda de archivos `asp` o `aspx` por su extensión debido a que es un `iis`.
...
Como me dio error, puede ser que se valide por detrás algún tipo de archivo o sanitización para evitar archivos maliciosos, por lo que ahora puedo interceptar la petición de la subida con  `burpsuite` para ir jugando con `bypasses`.

![image-center](/assets/images/{54787F35-70DC-4049-8DF0-B9EA8A5C631D}.png)

Usando `bypasses` como la doble extensión no me funciono pero usando el `null byte` si `(%00)` y además proporcionándole la extensión de una imagen. Pero sin embargo cuando voy a la ruta `/uploadedfiles/shell.aspx` no me carga ni me devuelve una shell sino que me muestra el siguiente error.

![image-center](/assets/images/{337E8E45-D4AC-4F5A-8558-B2EC6322AAFE}.png)

### Web.config 

Generalmente los servicios `ASP` de Windows `(IIS)` usan archivos de configuración como el `Web.config` y existen archivos maliciosos en linea por ejemplo el de este conocido repositorio -> [enlace](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Upload%20Insecure%20Files/Configuration%20IIS%20web.config/web.config) de `PayloadAllTheThings`, el que ya directamente crea una interfaz mas linda para poder ejecutar comandos.
...
Lo que hace este fragmento de código es ejecutar con `cmd` un comando el cual carga en memoria lo que descargue del recurso que se ubica en la `ip` `10.10.17.19` ósea el archivo `shell.ps1` el cual lo renombre así para tener un nombre mas cómodo, pero viene siendo el `Invoke-PowerShellTcp.ps1` que se puede descargar de este [enlace](https://github.com/samratashok/nishang/blob/master/Shells/Invoke-PowerShellTcp.ps1).
...
También lo descargo en la maquina y ahora con `msfconsole` configuro el `meterpreter`.
