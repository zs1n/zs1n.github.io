...
## CVE 
Como se que corre `laravel-php` en la pagina, encontré este [PoC](https://flyd.uk/post/cve-2023-24249/) asociado a la versión que corre para la misma.

![image-center](/assets/images/{53FE7BD8-9388-4CB8-9FF4-4ECBD3C30B66}.png)
## Shell as dash

Como veo en el mismo post, indica que el filtro aplica para archivos con extensión `php`.

![image-center](/assets/images/{6720C36B-0DC8-4D93-A439-2E5F1CAC1D50}.png)
### Bypass

Pero usando la doble extensión, veo que si, el archivo se sube.

![image-center](/assets/images/{23B39D80-46B3-4C60-BDCE-D3A23F18DA4F}.png)

Probando ejecución de comandos, veo que el usuario que corre por detrás es `dash`.
...
Por lo que me envié una shell a mi maquina con el siguiente comando:
...
Viendo los archivos los cuales tiene `xander` la posibilidad de ejecutarlos como `root` encontré el siguiente.
...
Además viendo el contenido del mismo con `strings`, veo que lo que usa para hacer el backup es `7za`
```bash
...
Una forma de abusar de esto, es generando un archivo que lleve delante un `@` que en si, `7za` no lo comprime sino que lee el contenido del mismo para luego comprimir cada archivo que contenga dentro de el, ya que como en el mismo comando se ve, usa `--` la cual significa que cualquier archivo empezado con `@` o con `-` lo guarda como un archivo vacío y es acá cuando entra en juego el parámetro `-snl` que hace que el binario no guarde el contenido de la clave privada en el zip, ya que voy a crear un link simbólico de la `id_rsa` de `root` con un archivo cualquiera, haciendo que el contenido de la misma se muestre en pantalla.
...
Ahora ejecuto el binario y selecciono la opción `3`, y veo el contenido de la clave `rsa`.
