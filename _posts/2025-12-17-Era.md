...
O otra opción, desde nuestra consola con `curl`. 
...
Luego subimos el archivo final a la maquina montándonos un servidor en `Python` y descargándolo desde la misma con o `wget`.
...
Donde extraigo la sección `.text_sig` del binario `original` para insertarlo en el `duplicado`, de lo contrario va a fallar.
