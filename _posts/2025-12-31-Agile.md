...
![[Pasted image 20260101020650.png]]
### Login

Me registre e inicie sesion.

![[Pasted image 20260101020816.png]]
...
![[Pasted image 20260101022435.png]]

Interceptando la petición al hacer click en `Export`, veo que se devuelve el contenido de un archivo.

![[Pasted image 20260101022419.png]]
## Shell as www-data

Probando una simple inyección para un `LFI` logre ver el contenido del `/etc/passwd`.

![[Pasted image 20260101022507.png]]

### Environ

Viendo el contenido de `/proc/self/environ` veo que la pagina se corre como el usuario `www-data`.

![[Pasted image 20260101022817.png]]
### MySQL credentials

Y además un archivo de configuracion, con claves de conexion a una base de datos de `MySQL`.

![[Pasted image 20260101022906.png]]
...
![[Pasted image 20260101023514.png]]
### Craft PIN

Para poder obtener el mismo segui la guía de este [repositorio](https://github.com/wdahlenburg/werkzeug-debug-console-bypass), el cual usando el propio `LFI` puedo obtener todos los datos.

El path de la aplicacion lo obtengo de la pagina de error.

![[Pasted image 20260102003500.png]]
### Private bits

Luego para obtener los `Private Bits`, necesito primero la dirección `MAC` la cual obtengo del siguiente archivo :
...
Y luego el `appname` que como indica [acá](https://book.hacktricks.wiki/en/network-services-pentesting/pentesting-web/werkzeug.html) , generalmente resuelve a `Flask`, o `wsgi_app`, de todas formas esto lo puedo averiguar viendo el contenido del `app.py`.

![[Pasted image 20260102004243.png]]
### Script

De manera que mi script queda así:
...
Colocándolo, es correcto, así que me envié una `shell` con el siguiente comando.

![[Pasted image 20260101034005.png]]
...
![[Pasted image 20260102010110.png]]

Yendo al link que me provee el propio `devtools` me salen el contenido de dicho archivo, junto con unas credenciales de conexión para una base de datos, distintas a las credenciales que obtuve antes.

![[Pasted image 20260102010336.png]]
...
La versión sudo es anterior a `1.9.12p2`, por lo que `sudoedit` es vulnerable a `CVE-2023-22809`. Para explotar esta vulnerabilidad, necesito un archivo que el `dev_admin` puede crear, y `root` puede ejecutar.

Así que corrí `pspy64` para encontrar algún proceso con estas características y q corran como `root`.
...
![[Pasted image 20260102011451.png]]
### Shell
