...
>Los servidores `DNS` no deberían permitir transferencias de zona (`AXFR`) a cualquier persona, ya que revela la estructura completa del dominio. Si está mal configurado, es una gran ganancia para la enumeración.
...
```bash
──(root㉿zsln)-[/home/…/Desktop/zsln/htb/trick]
...
>**Fail2ban** es una aplicación de software diseñada para la **prevención de intrusiones** en sistemas operativos, especialmente en servidores Linux.
   Su función principal es proteger los servicios de red de un sistema (como SSH, FTP, o servidores web) contra **ataques de fuerza bruta** y otros intentos de acceso no deseado.
